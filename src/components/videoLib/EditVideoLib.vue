<template>
    <div class="edit-video-lib-container" v-show="show">
      <div @click="cancelEdit()" class="mask-layer"></div>
      <div class="content">
        <el-form :model="form" label-width="100px">
          <el-form-item label="视频名" v-show="form.id">
            <span class="song-name">{{form.preVideoName}}</span>
          </el-form-item>
          <el-form-item label="视频封面" v-show="form.id">
            <img class="img-face" :src="form.videoImg" alt="">
          </el-form-item>
          <el-form-item label="上传视频" v-show="!form.id">
            <div class="upload-box">
              <el-button>上传<i class="el-icon-upload el-icon--right"></i></el-button>
              <input type="file" @change="changeUpload()" ref="uploadFile" class="op-input" accept="video/mp4" multiple>
            </div>
            <div class="file-name">
              <div class="name-box">
          ...
          <el-form-item label="添加视频封面" v-show="!form.id">
      ...
      methods: {
        ...
        confirm(){
          ...
          for (var i = 0; i < this.uploadFileList.length; i++){
            addParam.append("videoName", this.uploadFileList[i]);
          }
          addParam.append("type", type);
          addParam.append("remark", this.form.remark || "");
          addParam.append("videoImg", file, file.name);
          var editParam = {
            id: this.form.id,
            type: type,
          ...
          this.$axios.post("/addVideoLib", dataList).then( res => {
