<template>
    <div class="edit-square-container" v-show="show">
...
<script>
    export default {
      ...
      methods: {
        ...
        changeUpload(){
          this.form.squareImg = window.URL.createObjectURL(this.$refs.uploadFile.files[0]);
        ...
        confirm(){
          ...
          if(type === "0"){
            ...
            addParam.append("authorId", this. $store.state.userId);
            addParam.append("squareImg", file, file.name);
          }else{
            editParam.append("id", this.form.id);
            editParam.append("type", type);
            editParam.append("text", this.form.text);
            file === undefined ? editParam.append("squareImg", "") : editParam.append("squareImg", file, file.name);
          }
          var dataList = this.form.id ? editParam : addParam;
          this.$axios.post("/addSquare", dataList).then( res => {
